// constans
var POSSIBLE_WORDS = ["obdurate", "varisimilitude", "denfenestrate","obsequious", "dissonant", "toady", "idempotent"];
...
function guessLetter() {
    ...
    guesses.indexOf(letter) >= 0) {
        return; //game is over, or already this letter
    }
    guesses += letter;
...
function updatePage() {
    //updat clue string such as "h _ l l _ "
    var clueString = "";
    for (var i = 0; i< word.length; i++) {
        var letter = word.charAt(i);
        if (guesses.indexOf(letter) >= 0) {  //letter has been guessesd
            clueString += letter + " ";
        } else {                             //not guessed
            clueString += "_ ";
        }
